...
public class PrecisionScorer extends MetricScorer {
	...
	public double score(RankList rl)
	{
		...
		{
			if(rl.get(i).getLabel() == 2)//means the precision of relevant rank in top k 
				count++;
		}
		return ((double)count)/size;
	}
	public double myScore(RankList rl)
	{
		int count = 0;
		...
		if(k > rl.size() || k <= 0)
			size = rl.size();
		if (rl.get(0).getLabel() == 2)
		{
			count++;
		}
		for(int i=1;i<size;i++)
		{
			if(rl.get(i).getLabel() == 1)//exception the first one, the rest has label 1 
				count++;
		}
		return ((double)count)/size;
	}
