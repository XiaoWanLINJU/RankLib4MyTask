...
public class Evaluator {
	...
	public static void main(String[] args) {
		...
		for(int i=0;i<args.length;i++)
		{
			if(args[i].compareTo("-train")==0)
				trainFile = args[++i];
			else if(args[i].compareTo("-pnorm")==0)
				pnorm = Integer.parseInt(args[++i]);
		...
		if(trainFile.compareTo("")!=0)
		{
			...
			else
			{
				if(ttSplit > 0.0)//we should use a held-out portion of the training data for testing?
					...
					e.evaluate(trainFile, validationFile, testFile, featureDescriptionFile, pnorm);//All files except for trainFile can be empty. This will be handled appropriately
			}
		}
	...
	private void evaluate(String trainFile, String validationFile, String testFile, String featureDefFile,
			int pnorm) {
		System.out.print("eval with pnorm");
		...
		Ranker ranker = trainer.train(type, train, validation, features, trainScorer, pnorm);
	...
	public void score4test(String modelFile, String testFile, String outputFile)
	{
		Ranker ranker = rFact.loadRankerFromFile(modelFile);
		int[] features = ranker.getFeatures();
		List<RankList> test = readInput(testFile);
		...
		try {
			...
			for(int i=0;i<test.size();i++)
			{
				...
				for(int j=0;j<l.size();j++)
				{
					out.write(l.getID() + "\t" + j + "\t" + ranker.eval4test(l.get(j))+"");
					out.newLine();
				}
			}
			out.close();
