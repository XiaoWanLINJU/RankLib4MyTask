...
public class RankerTrainer {
	...
	public Ranker train(RANKER_TYPE type, List<RankList> train, List<RankList> validation, int[] features, MetricScorer scorer, int pnorm)
	{
		Ranker ranker = rf.createRanker(type, train, features, scorer, pnorm);
		ranker.setValidationSet(validation);
		long start = System.nanoTime();
		ranker.init();
		ranker.learn(pnorm);
