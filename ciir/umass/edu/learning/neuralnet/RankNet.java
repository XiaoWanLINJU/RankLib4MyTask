...
public class RankNet extends Ranker {
	...
	protected int[][] batchFeedForward(RankList rl)
	{
		...
		return pairMap;//一個list中比第i個壞的instance
	}
	...
	private void batchBackPropagate(int[][] pairMap, float[][] pairWeight, int pnorm) {
		for(int i=0;i<pairMap.length;i++)
		{
			//back-propagate
			PropParameter p = new PropParameter(i, pairMap);
			outputLayer.computeDelta(p, pnorm);//starting at the output layer
			for(int j=layers.size()-2;j>=1;j--)//back-propagate to the first hidden layer
				layers.get(j).updateDelta(p);
	...
	protected double crossEntropy(double o1, double o2, double targetValue)//learning to rank using gradient decent 
	{
		double oij = o1 - o2;
		double ce = -targetValue * oij + SimpleMath.logBase2(1+Math.exp(oij));
		return (double) ce;
	}
	protected double pnormCrossEntropy(double o1, double o2, double targetValue, int p)//learning to rank using gradient decent 
	{
		double oij = o1 - o2;
		double ce = Math.pow(-targetValue * oij + SimpleMath.logBase2(1+Math.exp(oij)), (double)p);
		return (double) ce;
	}
	...
	protected void pnormEstimateLoss(int pnorm) 
	{
		misorderedPairs = 0;
		error = 0.0;
		for(int j=0;j<samples.size();j++)
		{
			...
			for(int k=0;k<rl.size()-1;k++)
			{
				...
				for(int l=k+1;l<rl.size();l++)
				{
					...
					{
						double o2 = eval(rl.get(l));
						error += pnormCrossEntropy(o1, o2, 1.0f,pnorm);
						if(o1 < o2)
							misorderedPairs++;
					}
				}
	...
	public void learn()
	{
		...
		for(int i=1;i<=nIteration;i++)
		{
			...
			//printWeightVector();
			scoreOnTrainingData = scorer.myScore(rank(samples));
			estimateLoss();
			...
			if(i % 1 == 0)
			{
				...
				if(validationSamples != null)
				{
					double score = scorer.myScore(rank(validationSamples));
		...
		scoreOnTrainingData = SimpleMath.round(scorer.myScore(rank(samples)), 4);
		PRINTLN("--------------------------------------------------");
		PRINTLN("Finished sucessfully.");
		PRINTLN(scorer.name() + " on training data: " + scoreOnTrainingData);
		if(validationSamples != null)
		{
			bestScoreOnValidationData = scorer.myScore(rank(validationSamples));
			PRINTLN(scorer.name() + " on validation data: " + SimpleMath.round(bestScoreOnValidationData, 4));
		}
		PRINTLN("---------------------------------");
	}
	@Override
	public void learn(int pnorm) {
		System.out.print("rantnet learn with pnorm");
		PRINTLN("-----------------------------------------");
		PRINTLN("Training starts...");
		PRINTLN("--------------------------------------------------");
		PRINTLN(new int[]{7, 14, 9, 9}, new String[]{"#epoch", "% mis-ordered", scorer.name()+"-T", scorer.name()+"-V"});
		...
		for(int i=1;i<=nIteration;i++)
		{
			for(int j=0;j<samples.size();j++)
			{
				...
				int[][] pairMap = batchFeedForward(rl);
				float[][] pairWeight = computePairWeight(pairMap, rl);
				batchBackPropagate(pairMap, pairWeight,pnorm);
				clearNeuronOutputs();
			}
	...
	//在test中計算得分，不用sigmoid，因為會有很多doc的得分是1.0，直接輸出得分，看是否可以區分
	public double eval4test(DataPoint p)
	{
		//feed input
		for(int k=0;k<inputLayer.size()-1;k++)//not the "bias" node
			inputLayer.get(k).setOutput(p.getFeatureValue(features[k]));
		...
		inputLayer.get(inputLayer.size()-1).setOutput(1.0f);		
		//propagate
		for(int k=1;k<layers.size() - 1;k++)//skip the input layer
			layers.get(k).computeOutput();	
		//outputlayer has one neuron
		outputLayer.get(0).computeOutput4test();	
		return outputLayer.get(0).getOutput();
	}	
