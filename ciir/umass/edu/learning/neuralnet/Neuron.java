...
public class Neuron {
	...
	public void computeOutput4test()
	{
		Synapse s = null;
		double wsum = 0.0;
		for(int j=0;j<inLinks.size();j++)
		...
		output = wsum;//using the specified transfer function to compute the output
	}	
	...
	public void computeDelta(PropParameter param)
	{
		...
		for(int k=0;k<pairMap[current].length;k++)
		{
			...
			{
				weight = 1;//之前算出只有這些instance的輸出需要計算以便更新
	...
	/**
	 * Compute delta for neurons in the output layer with the loss function is p-norm. ONLY for neurons in the output layer.
	 * @param targetValue
	 */
	public void computeDelta4pnorm(PropParameter param, int pnorm)
	{
		/*double pij = (double) (1.0 / (1.0 + Math.exp(-(prev_output-output))));
		prev_delta = (targetValue-pij) * tfunc.computeDerivative(prev_output);
		delta =      (targetValue-pij) * tfunc.computeDerivative(output);*/
		...
		for(int k=0;k<pairMap[current].length;k++)
		{
			...
			{
				weight = 1;//之前算出只有這些instance的輸出需要計算以便更新
				pij = (double) pnorm * (-(outputs.get(current)-outputs.get(j) + Math.log(1 + Math.exp(outputs.get(current)-outputs.get(j))))) * (1.0 / (1.0 + Math.exp(outputs.get(current)-outputs.get(j))));//this is in fact not "pij", but "targetValue-pij":  1 - 1/(1+e^{-o_ij})
			}
			else//LambdaRank
			{
				weight = param.pairWeight[current][k];
